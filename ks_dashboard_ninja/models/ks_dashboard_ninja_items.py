...
class KsDashboardNinjaItems(models.Model):
        ...
        for rec in self:
            if rec.ks_record_count_type == 'count':
                ...
                if len(ks_records_grouped_data)>0:
                    ...
                    if rec.ks_record_count_type == 'sum' and ks_records_grouped_data.get('__count',False) and (ks_records_grouped_data.get(rec.ks_record_field.name)):
        ...
        for rec in self:
            ...
            if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and rec.ks_dashboard_item_type != 'ks_list_view' and rec.ks_model_id and rec.ks_chart_data_count_type:
                ...
                if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                    ...
                    elif rec.ks_chart_groupby_type == 'selection' and ks_chart_groupby_relation_field:
                        ks_chart_records = self.env[rec.ks_model_name].read_group(ks_chart_domain, set(
                            ks_chart_measure_field + ks_chart_measure_field_2 + [ks_chart_groupby_relation_field]),
                                                                                  [ks_chart_groupby_relation_field],
                                                                                  orderby=orderby, limit=limit)
        ...
        if (ks_date_data):
            ...
            if rec.ks_record_count_type == 'count':
                ...
                if rec.ks_record_count_type == 'sum':
                    return data.get(rec.ks_record_field.name,0) if data.get('__count',False) and (data.get(rec.ks_record_field.name)) else 0
                else:
                    return data.get(rec.ks_record_field.name,0)/data.get('__count',1) if data.get('__count',False) and (data.get(rec.ks_record_field.name)) else 0
            else:
                return False
        else:
            return False
    ...
    def ks_get_model_2_data(self, ks_model_name_2, ks_record_count_type_2, ks_record_field_2, ks_domain, rec):
        ...
        elif ks_record_count_type_2 in ['sum', 'average'] and ks_record_field_2:
            ks_records_grouped_data = rec.ks_fetch_model_data(ks_model_name_2, ks_domain, 'read_group', rec)
            if len(ks_records_grouped_data) > 0:
                ks_records_grouped_data = ks_records_grouped_data[0]
                if rec.ks_record_count_type == 'sum' and ks_records_grouped_data.get('__count', False) and (ks_records_grouped_data.get(ks_record_field_2.name)):
